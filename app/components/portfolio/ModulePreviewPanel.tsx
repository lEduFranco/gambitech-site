...
import { getAccentText, getBrowserHostname } from "../../data/portfolio";
import { getModuleIcon } from "./module-icons";

type ModulePreviewPanelProps = {
  project: PortfolioProject;
  variant?: "preview" | "hero" | "thumb";
};

export function ModulePreviewPanel({ project, variant = "preview" }: ModulePreviewPanelProps) {
  const modules = project.modules;
  if (!modules?.length) return null;

  const accentText = getAccentText(project);
  const hostname = getBrowserHostname(project);
  const visibleModules =
    variant === "thumb" ? modules.slice(0, 4) : variant === "preview" ? modules.slice(0, 5) : modules;

  const chromeTitle =
    variant === "thumb"
      ? `Em breve · ${project.name}`
      : `em breve · ${hostname ?? project.name.toLowerCase()}`;
    ...
    >
      ...
      >
        ...
        >
          ...
          {variant !== "hero" && modules.length > visibleModules.length && (
            <p
              className={`text-center text-gray-500 ${
                variant === "thumb" ? "text-[9px] pt-0.5" : "text-[11px] pt-1"
              }`}
            >
              +{modules.length - visibleModules.length} módulos
            </p>
          )}
        </div>
        ...
        {variant !== "thumb" && (
          ...
          >
            <Clock className="w-3 h-3" style={{ color: accentText }} />
            <span className="text-[9px] text-gray-500">
              {modules.length} módulos · Em desenvolvimento
            </span>
          </div>
        )}
      </div>
